...
TEST_F(MmuBasicTb, ByteEnableSingleByteWrite) {
    uint32_t addr = 0x00000100;
    int cycles = 0;

    writeWord(addr, 0x11223344, cycles);          // baseline word
    writeWord(addr, 0x0000AA00, cycles, 0b0010);  // update only byte[15:8]

    bool hit = false;
    uint32_t r = readWord(addr, cycles, hit);
    EXPECT_EQ(r, 0x1122AA44u) << "Only selected byte should change";
}

TEST_F(MmuBasicTb, ByteEnableHalfwordWrite) {
    uint32_t addr = 0x00000120;
    int cycles = 0;

    writeWord(addr, 0x01020304, cycles);          // baseline word
    writeWord(addr, 0xBEEF0000, cycles, 0b1100);  // update upper halfword

    bool hit = false;
    uint32_t r = readWord(addr, cycles, hit);
    EXPECT_EQ(r, 0xBEEF0304u) << "Only upper halfword should change";
}
