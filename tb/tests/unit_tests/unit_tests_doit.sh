...
# --- Determine the script's and testbench's location ---
...
# --- Resolve the RTL path relative to the Testbench directory (Crucial Fix) ---
TB_DIR=$(dirname "$TB_PATH")

# Temporarily change directory to correctly resolve the relative RTL path
...
if [[ ! -f "$RTL_PATH" ]]; then
    ...
    echo "Tried resolving relative to $TB_DIR"
    echo "Resolved path: $RTL_PATH"
    exit 1
fi

# --- Determine the necessary search path for sub-modules ---
# This directory contains control_unit.sv, main_decoder.sv, and alu_decoder.sv
RTL_DIR=$(dirname "$RTL_PATH") 
...
# --- Run ---
echo "Running tests for $TOP_MODULE..."
./obj_dir/"$BIN"
